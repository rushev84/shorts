...
function getRandomInt() {
  const min = 117
  const max = 132
  out1.textContent = Math.floor(min + Math.random() * (max + 1 - min))
}
...
function getRandomInt2() {
  out2.textContent = Math.floor(min + Math.random() * (max + 1 - min))
}
...
function t6() {
  ...
  max = 10099
  getRandomInt2()
  let rand = document.querySelector('.out-2').innerText
  if (rand % 2 === 0) {
    even()
  } else {
    odd()
...
// Task 9
// Напишите функцию t9, которая в зависимости от значения переменной z9, делает активным option s-9 с таким же value. Внимание, z9 может иметь значения 1, 2, 3, 4, 5. Действия запускаются по кнопке .b-9.
