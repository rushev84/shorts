...
function t2(min, max, block) {
  block.textContent = Math.floor(min + Math.random() * (max + 1 - min))
}
...
function t3(min = 0, max = 100) {
  out3.textContent = Math.floor(min + Math.random() * (max + 1 - min))
}
