...
function createSlider(event) {
  ...
  for (let i = 0; i < aphorisms.length; i++) {
    ...
    if (i === 0) div.style.background = bgColor[randomInt(0, bgColor.length - 1)]
    let text = document.createElement('div')
    text.textContent = aphorisms[i]
    div.append(text)
    outField.append(div)
  }
  ...
  outField.addEventListener('click', () => {
    hideSlide(count)
    count + 1 < aphorisms.length ? count++ : (count = 0)
    showSlide(count)
  })
...
const hideSlide = (count) => {
  slideCollection[count].classList.add('hide')
}

const showSlide = (count) => {
  slideCollection[count].classList.remove('hide')
  slideCollection[count].style.background = bgColor[randomInt(0, bgColor.length - 1)]
}
...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelector('.btn-start').addEventListener('click', createSlider)
})
