...
function t1(elem) {
  ...
  function r1(elem) {
    out += elem.tagName + ' '
    Array.from(elem.children).forEach((item) => r1(item))
  }
...
function t2(elem) {
  ...
  function r2(elem) {
    ...
    if (num) s += num
    Array.from(elem.children).forEach((item) => r2(item))
  }
...
function t3(elem) {
  ...
  function r3(elem) {
    let needElements = ['SPAN', 'B', 'I', 'P']
    // проверяем наличие детей
    // если нет - то наш вариант
    console.dir(elem)
    if (elem.children.length === 0 && needElements.includes(elem.tagName)) {
      out += elem.textContent
    }
    if (elem.children.length === 0 && !needElements.includes(elem.tagName)) {
      out += elem.outerHTML
    }
    Array.from(elem.children).forEach((item) => r3(item))
  }
...
function t4(elem) {
  ...
  function r4(elem) {
    ...
    Array.from(elem.children).forEach((item) => r4(item))
  }
}
...
function t5(elem) {
  ...
  function r5(elem) {
    if (elem.children.length === 0) {
      ...
      elem.parentElement.insertBefore(markElem, elem)
      markElem.textContent = elem.textContent
      elem.parentElement.removeChild(elem)
    }

    Array.from(elem.children).forEach((item) => r5(item))
  }
}
