...
function t4() {
  const radio = document.querySelectorAll('.r-4')
...
function t5() {
  ...
  return eval(s51.value + s52.value + s53.value)
}
...
function t21(s) {
  while (true) {
    let num = t13(10, 20)
    if (s === 'odd' && num % 2 !== 0) return num
    if (s === 'even' && num % 2 === 0) return num
  }
}
...
function t26() {
  let userName = document.querySelector('.i-26').value
  if (userName.trim() !== '') {
    document.querySelector('.out-26').textContent = userName.toLowerCase()
  }
}
