...
const f3 = () => {
  document.querySelector('.out-3').textContent = a3.every((item) => sum(item) > 20)
}
...
const f5 = () => {
  document.querySelector('.out-5').textContent = a5.every(
    (item) => (item.age >= 16 && item.pay === true) || item.age < 16
  )
}
