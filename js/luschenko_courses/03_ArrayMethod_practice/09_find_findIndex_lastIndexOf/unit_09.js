...
const f6 = () => {
  ...
  while (tempIndex !== -1) {
    tempIndex = subArr.findIndex((item) => {
      if (item > a6_from && item < a6_to) {
        return true
      }
    })
    res += tempIndex + 1
    subArr = subArr.slice(tempIndex + 1)
  }
...
const f7 = () => {
  document.querySelector('.out-7').textContent = str7.lastIndexOf(a7_1)
}
...
const f8 = () => {
  let res1 = str8.lastIndexOf(a8_1.toUpperCase())
  let res2 = str8.lastIndexOf(a8_1.toLowerCase())
  if (res1 < res2) {
    res1 = res2
  }
