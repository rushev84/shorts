...
const f1 = () => {
  ...
  out1.textContent = a1.includes(value)
}
...
const f2 = () => {
  ...
  if (a2.includes(value)) {
    out2.textContent = a2.indexOf(value)
  } else {
...
const f5 = () => {
  ...
  let from = +document.querySelector('.i-5-2').value
  if (a5.includes(value, from)) {
...
// TASK 06
// По нажатию b-6 выполняется функция f6. Функция считывает строку из i-6 и с помощью includes и ищет данный элемент в массиве a6. Выводит в out-6 false если искомой строки нет в a6 и true если есть.
...
const f6 = () => {
  ...
  out6.textContent = a6.includes(value)
}
...
const f8 = () => {
  ...
  if (a8.includes(value)) {
    out8.textContent = true
  } else {
    out8.textContent = false
  }
}
...
const f9 = () => {
  ...
  let result = false
  if (Number(value)) {
    if (a9.includes(Number(value))) result = true
  } else {
    if (a9.includes(value.toUpperCase()) || a9.includes(value.toLowerCase())) result = true
  }
