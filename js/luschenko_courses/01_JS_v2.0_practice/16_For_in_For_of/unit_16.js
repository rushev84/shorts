...
function f6() {
  ...
  a6 = arr
  // a6 стал одномерным массивом. Out можно было делать в предыдущем цикле, но тогда массив a6 не изменится.
  let out = ''
  for (let item of a6) {
    out += item + ' '
  }
...
function f9() {
  let maxLength = 0
  for (let key in a9) {
    if (a9[key].length > maxLength) maxLength = a9[key].length
  }
  return maxLength - 1
}
...
function f15() {
  const out15 = document.querySelectorAll('.out-15')
