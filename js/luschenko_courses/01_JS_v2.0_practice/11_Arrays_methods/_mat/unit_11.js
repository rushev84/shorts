...
function f13() {
  for (let i = 0; i < Math.floor(d13.length / 2); i++) {
    let temp = d13[i]
    d13[i] = d13[d13.length - i - 1]
    d13[d13.length - i - 1] = temp
  }
  showArr('.out-13', d13)
}
...
function f14() {
  // при повторном нажатии на кнопку возможны два варианта действий:
  // 1. добавлять в уже существующий массив новые элементы (сейчас работает так)
  // 2. очищать массив и только после этого добавлять (тогда надо дописать в начале функции d14.length = 0)
...
function f15() {
  let element = +document.querySelector('.i-15').value
  if (d15.indexOf(element) === -1) d15.push(element)
  showArr('.out-15', d15)
}
...
function f18() {
  let element = document.querySelector('.i-18').value
  document.querySelector('.out-18').textContent = d18.includes(element)
}
