...
const f7 = () => {
  ...
  const s7 = new Set(i7)
  s7.size === i7.length && i7.length > 6 ? (out7.textContent = 1) : (out7.textContent = 0)
}
...
const f12 = () => {
  const strAr = str12.split('')
  const strSet = new Set(strAr)
  return strSet
  // Кажется, нет необходимости создавать массив для выполнения данной задачи.
  // Так тоже работает - const strSet = new Set(str12)
}
...
const f13 = () => {
  const strAr = str13.split('')
  const strSet = new Set(strAr)
  const uniqueSymbols = {}

  for (let item of strSet) {
    let counter = 0
    for (let keyAr in strAr) {
      if (item === strAr[keyAr]) counter++
    }
    uniqueSymbols[item] = counter
  }
