...
function t10() {
  const a10_res = []
  a10.forEach((elem) => {
    elem >= 0 ? a10_res.push(elem) : a10_res.push(0)
  })
  a10 = a10_res
  return a10
}
...
function t14() {
  a14_symLowerCase = a14_sym.toLowerCase()
  const a14lowerCase = []
  a14.forEach((elem) => {
    a14lowerCase.push(elem.toLowerCase())
  })
  if (a14lowerCase.includes(a14_symLowerCase)) {
    return true
  } else {
    return false
  }
...
function t15() {
  a15_res = a15.filter((elem) => elem.pnum && elem.pnum.length === 6)
  return a15_res
}
