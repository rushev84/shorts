...
function f4() {
  let age = 2021 - +document.querySelector('.i-4').value
  console.log(age)
  if (age >= 18) {
    out4.textContent = 1
  } else {
...
function f10() {
  out10.textContent = document.querySelector('.s-100').value
}
...
function f11() {
  out11.textContent = document.querySelector('.s-110').value
}
...
// Я думаю, это  потому, что пользователь может вводить числа, но тип данных, передаваемый из инпута, по-прежнему остаётся string - просто строка состоит из чисел
...
// Данную задачу можно решить через switch/case, но есть более простое решение
...
function f14() {
  ...
  out14.textContent = eval(i141 + s + i142)
}
...
function f15() {
  ...
  out15.textContent = eval(s151 + s + s152)
}
