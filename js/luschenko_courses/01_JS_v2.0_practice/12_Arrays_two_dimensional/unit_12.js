...
function f2() {
  ...
  // везде, где в выводимой строке нет тэгов html, я использую textContent - он менее ресурсоёмкий
...
function f12() {
  ...
  // for (let i = 0; i < a12.length ** 2; i++) {
  //   if (a12[Math.floor(i / a12.length)][i % a12.length]) {
  //     out += '1 '
  //   }
  // }
  // В данном случае мы увеличиваем out не путем добавления элемента, равного единице, а путем добавления строки '1 '. Но внешне работа этого решения ничем не отличима от работы предыдущего.
  // Количество итераций остаётся таким же, как и в первом варианте - 64. Так что оптимизации здесь никакой, поэтому первое решение предпочтительней. К тому же код в нём более читабельный.
...
function f13() {
  for (let i = 0; i < 8; i++) {
    ...
    for (let k = 0; k < 8; k++) {
      if (k % 2) {
        i % 2 ? (a13[i][k] = 0) : (a13[i][k] = 1)
      } else {
        i % 2 ? (a13[i][k] = 1) : (a13[i][k] = 0)
      }
      // Аналогично записи:
      //   if (k % 2) {
      //     if (i % 2) {
